...
public class GradientBuffer {
    ...
    public void clear() {
        zero(grad_W_enc1); zero(grad_b_enc1);
        zero(grad_W_enc2); zero(grad_b_enc2);
        zero(grad_W_mu); zero(grad_b_mu);
        zero(grad_W_logvar); zero(grad_b_logvar);
        zero(grad_W_dec1); zero(grad_b_dec1);
        zero(grad_W_dec2); zero(grad_b_dec2);
        zero(grad_W_decOut); zero(grad_b_decOut);
    }
    ...
    public void scale(double factor) {
        scale(grad_W_enc1, factor); scale(grad_b_enc1, factor);
        scale(grad_W_enc2, factor); scale(grad_b_enc2, factor);
        scale(grad_W_mu, factor); scale(grad_b_mu, factor);
        scale(grad_W_logvar, factor); scale(grad_b_logvar, factor);
        scale(grad_W_dec1, factor); scale(grad_b_dec1, factor);
        scale(grad_W_dec2, factor); scale(grad_b_dec2, factor);
        scale(grad_W_decOut, factor); scale(grad_b_decOut, factor);
    }
    ...
    public void accumulate(GradientBuffer other) {
        add(grad_W_enc1, other.grad_W_enc1); add(grad_b_enc1, other.grad_b_enc1);
        add(grad_W_enc2, other.grad_W_enc2); add(grad_b_enc2, other.grad_b_enc2);
        add(grad_W_mu, other.grad_W_mu); add(grad_b_mu, other.grad_b_mu);
        add(grad_W_logvar, other.grad_W_logvar); add(grad_b_logvar, other.grad_b_logvar);
        add(grad_W_dec1, other.grad_W_dec1); add(grad_b_dec1, other.grad_b_dec1);
        add(grad_W_dec2, other.grad_W_dec2); add(grad_b_dec2, other.grad_b_dec2);
        add(grad_W_decOut, other.grad_W_decOut); add(grad_b_decOut, other.grad_b_decOut);
    }
